...
export class HostGame {
    ...
    // Take over an existing game when becoming host after previous host disconnected
    takeOverGame(existingState, disconnectedPlayerId) {
        console.log('[Host] Taking over game with existing state');
        
        if (!existingState) {
            console.error('[Host] No existing state to take over!');
            return;
        }

        // Deserialize the state back into a proper game state
        this.gameState = this._deserializeGameState(existingState);
        
        // Remove the disconnected player
        if (this.gameState.players[disconnectedPlayerId]) {
            removePlayer(this.gameState, disconnectedPlayerId);
        }

        this.isRunning = true;
        this.myPlayerId = this.socket.id;

        // Set up callbacks for game loop
        const callbacks = {
            onUpdate: (state, soundEvents = []) => {
                const serialized = serializeGameState(state);
                this.onGameUpdate?.(serialized);
            ...
            onGoalScored: (team) => {
                this.onSoundEvents?.([{ type: 'crowd' }]);
                this.peers.forEach(peer => {
                    peer.send({ type: 'sound-events', events: [{ type: 'crowd' }] });
                });
            },
            onCountdown: (data) => {
                this.onSoundEvents?.([{ type: 'countdown', isDramatic: data.isDramatic }]);
                this.peers.forEach(peer => {
                    peer.send({ type: 'sound-events', events: [{ type: 'countdown', isDramatic: data.isDramatic }] });
                });
            },
            onCountdownPause: (action) => {
                const event = { type: 'countdownControl', action };
        ...
        // Start the game loop with existing state
        this.intervals = startGame(this.gameState, callbacks);
        
        // Send initial state to confirm takeover
        const serialized = serializeGameState(this.gameState);
        this.onGameUpdate?.(serialized);
    }

    _deserializeGameState(serialized) {
        // The serialized state from the client should be usable directly
        // but we need to ensure it has all the required properties
        return {
            ...serialized,
            gameStarted: true,
            isGameOver: serialized.isGameOver || false,
            isPausedForGoal: serialized.isPausedForGoal || false,
            kickOff: serialized.kickOff || false,
            countdownActive: serialized.countdownActive || false
        };
    }
}
